...
def fieldList(panel, sizerG, fields, dbg=False):
    if dbg:
...
def addComboBox(panel, sizer, label, index, action, border=BORDER,
    ...
    actionResult = action()
    (indexList, choiceList, text) = actionResult[:3]
...
def formatData(cfg, formatList, dbg=False):
    ...
    for fmt in formatList:
        ...
        if dbg:
            print(f"{index:3d} {cf.configTable[index]:<20} {str(fieldType):<4}",
                  end="")
...
def printFormData(panel, sizerG, dbg=False):
    if not dbg:
        return
    ...
    for i, data in enumerate(panel.formData):
        ...
        if len(data) == 2:
            ...
            print("%-12s " % (cb.__class__.__name__), end="")
            txtLen = dc.GetTextExtent(txt.GetLabel())[0] + cb.GetSize()[0]
            print("%-20s %3d" % (cf.configTable[index], txtLen), end = " ")
...
def saveData(dialog, dbg=False):
    if dbg:
        print(f"saveData {dialog.GetLabel()}")
    changed = False
    txt = ""
    cfg = dialog.mf.cfg
    for data in dialog.formData:
        if len(data) == 2:
            (obj, index) = data
            if index is None:
                continue
            if isinstance(obj, wx.StaticText):
                txt = str(obj.GetLabel())
                continue
            if dbg:
                print(f"{txt:<20} {index:3d} {cf.configTable[index]:<20}",
                      end="")
            cfgData = cfg.getInfo(index)
            fieldData = obj.GetValue()
            if dbg:
                print(f" cfgData {cfgData:<6} fieldData {fieldData:<6}",
                      end="")
            if cfgData != fieldData:
                if dbg:
                    print(" *", end="")
                cfg.setInfo(index, fieldData)
                if dbg:
    ...
    #     (label, index) = fmt[:2]
    #     cfgData = cfg.getInfo(index)
    #     fieldData = dialog.fieldInfo[index]
    #     if dbg:
    #         print(f"{index:3d} {cf.configTable[index]:<20} " \
    #               f"{str(fieldType):<4} " \
    #               f"cfgData {cfgData} fieldData {fieldData}")
    #     if cfgData != fieldData:
    #         # dialog.fieldInfo[index] = val
    #         cfg.setInfoData(index, fieldData)
    #         dialog.sendData = True
    #         changed = True
    # return changed
...
def OnDialogShow(evt):
    ...
    dialogShow(dialog, dialog.IsShown())

def dialogShow(dialog, shown):
    print(f"dialogShow {shown}")
    changed = False
    if shown:
...
class MoveCommands():
    ...
    def queMove(self, op, val=0):
        if self.send:
            ...
            strVal = f"{val:.4f}" if isinstance(val, float) else str(val)
    ...
    def queFeed(self, feed):
        self.queMove(en.Q_SAVE_FEED, feed)
    ...
    def moveZ(self, zLocation, flag=en.CMD_MAX, backlash=0.0):
    ...
    def moveX(self, xLocation, flag=en.CMD_MAX, backlash=0.0):
        if (flag & ct.DRO_POS) == 0:
...
class SendData:
    ...
    def sendClear(self):
        try:
            self.comm.command(cm.C_CLR_DBG)
    ...
    def sendDone(self):
        self.comm.command(cm.C_SEND_DONE)
    ...
    def sendZData(self, send=False):
        ...
        try:
            ...
            if send or (not self.zDataSent):
                ...
                queParm(pm.Z_DIR_INV, cfg.getBoolInfoData(cf.zInvDir))
                queParm(pm.Z_MPG_INV, cfg.getBoolInfoData(cf.zInvMpg))
    ...
    def sendXData(self, send=False):
        ...
        try:
            ...
            if send or (not self.xDataSent):
                ...
                queParm(pm.X_DIR_INV, cfg.getBoolInfoData(cf.xInvDir))
                queParm(pm.X_MPG_INV, cfg.getBoolInfoData(cf.xInvMpg))
                queParm(pm.X_LIM_NEG_INV, cfg.getBoolInfoData(cf.xLimNegInv))
                queParm(pm.X_LIM_POS_INV, cfg.getBoolInfoData(cf.xLimPosInv))
                queParm(pm.X_HOME_INV, cfg.getBoolInfoData(cf.xHomeInv))
...
def getSyncParm(control, feed, rpm, axis):
    ...
    if not FPGA:
        ...
        if turnSync == en.SEL_TU_SYN:
            syn = mf.zSyncInt if axis == AXIS_Z else mf.xSyncInt
            (control.cycle, control.output, control.inPreScaler) = \
                syn.calcSync(val, metric=False, rpm=rpm, turn=True)
    ...
    if not FPGA:
        if turnSync == en.SEL_TU_ISYN or \
           ...
           ((turnSync == en.SEL_TU_ESYN) and SYNC_SPI):
            ...
            queParm(pm.L_SYNC_IN_PRE_SCALER, control.inPreScaler)
            queParm(pm.L_SYNC_OUT_PRE_SCALER, control.outPreScaler)
            if SYNC_SPI:
                m = control.m
                m.syncParms()
...
class Turn(LatheOp, UpdatePass):
    ...
    def setup(self, add=False): # turn
        ...
        comm.command(cm.C_CMD_SYNC_SETUP)
    ...
    def runPass(self, addPass=False): # turn
        m = self.m
        flag = (en.CMD_JOG | ct.DRO_POS | ct.DRO_UPD) if X_DRO_POS else \
            en.CMD_MOV
...
class Arc(LatheOp, UpdatePass):
    ...
    def movePreCCW(self):
        ...
        m.moveX(self.xStart) #, en.CMD_SYN)
...
class Taper(LatheOp, UpdatePass):
    ...
    def externalRunPass(self, addPass=False):
        ...
        if self.zBackInc != 0.0:
            ...
            m.moveZ(self.startZ, en.CMD_JOG) # move to takeout backlash
        else:
            m.moveZ(self.startZ)
        if self.pause:
        ...
        if self.taperX:
            ...
            m.moveX(self.startX, en.CMD_SYN)
            if DRO:
                m.saveZDro()
                m.saveXDro()
    ...
    def internalRunPass(self, addPass=False):
        ...
        m.moveZ(self.startZ, en.CMD_JOG) # back to start to remove backlash
        m.moveX(self.startX, en.CMD_SYN)
        if self.pause:
            m.quePause(ct.PAUSE_ENA_X_JOG | ct.PAUSE_READ_X if addPass else 0)
        if not addPass and m.passNum & 0x300 == 0:
...
class ScrewThread(LatheOp, UpdatePass):
    ...
    def getParameters(self):
        ...
        calcSync = self.mf.zSyncInt.calcSync
        if not FPGA:
            if threadSync == en.SEL_TH_ISYN:
                (self.cycle, self.output, self.inPreScaler, self.outPreScaler) = \
                    calcSync(thVal, dbg=True, metric=metric, rpm=rpm)

            elif threadSync == en.SEL_TH_ESYN:
                (self.cycle, self.output, self.inPreScaler, self.outPreScaler) = \
                    calcSync(thVal, dbg=True, metric=metric, rpm=rpm)
        else:
            if threadSync == en.SEL_TH_SYN:
                (self.cycle, self.output, self.inPreScaler) = \
                    calcSync(thVal, dbg=True, metric=metric, rpm=rpm)
        ...
        print("not sure about this depth calculation")
    ...
    def setup(self, add=False): # thread
        ...
        if not FPGA:
            if threadSync == en.SEL_TH_ISYN or \
                ...
                queParm(pm.L_SYNC_IN_PRE_SCALER, self.inPreScaler)
                queParm(pm.L_SYNC_IN_PRE_SCALER, self.outPreScaler)
                if SYNC_SPI:
                    m.syncParms()
                    m.syncCommand(sc.SYNC_SETUP)
            elif threadSync == en.SEL_TH_ESYN:
            ...
            if  runoutSync == en.SEL_RU_ISYN or \
                threadSync == en.SEL_RU_SYN:
                ...
                queParm(pm.L_X_SYNC_OUT_PRE_SCALER, self.xOutPreScaler)
        else:
            if threadSync == en.SEL_TH_ENC:
                pass
            elif threadSync == en.SEL_TH_SYN:
                queParm(pm.L_SYNC_IN_PRE_SCALER, self.inPreScaler)
                queParm(pm.L_SYNC_CYCLE, self.cycle)
                queParm(pm.L_SYNC_OUTPUT, self.output)
    ...
    def runPass(self, addPass=False): # thread
        ...
        if self.rightHand:      # right hand threads
            flag = en.CMD_JOG | ((ct.DRO_POS | ct.DRO_UPD) if X_DRO_POS else 0)
            m.moveX(self.curX, flag)
        else:                   # left hand threads
            if self.runoutDist != 0:
                ...
                flag = en.CMD_JOG | ((ct.DRO_POS | ct.DRO_UPD) \
                                     if X_DRO_POS else 0)
                m.moveX(self.xStart, flag)
                
                m.moveX(self.curX, en.CMD_SYN)
        ...
        flag = en.CMD_SYN | ct.ENA_THREAD
        flag |= ct.SYN_START if self.rightHand else ct.SYN_LEFT
        if self.runoutDist != 0:
            flag |= ct.ENA_RUNOUT
        m.moveZ(self.endZ, flag)
    ...
    def jogZ(self, code, val):
        if val == 0:
            ...
            try:
                ...
                self.mf.comm.command(cm.C_Z_J_SPEED)
            except CommTimeout:
                commTimeout(self.mf.jogPanel)

    def jogX(self, code, val):
        if val == 0:
            self.jogDone(cm.C_X_STOP)
            print("jogX done")
            stdout.flush()
        else:
            index = abs(val)
            ...
            try:
                ...
                self.mf.comm.command(cm.C_X_J_SPEED)
            except CommTimeout:
                commTimeout(self.mf.jogPanel)
...
class JogPanel(wx.Panel, FormRoutines):
    ...
    def OnZSafe(self, _):
        if self.addEna:
            ...
            self.comm.queParm(pm.Z_FLAG, en.CMD_MAX)
            self.comm.command(cm.C_Z_MOVE_ABS)
        self.combo.SetFocus()
    ...
    def OnXSafe(self, _):
        if self.addEna:
            ...
            self.comm.queParm(pm.X_FLAG, en.CMD_MAX)
            self.comm.command(cm.C_X_MOVE_ABS)
        self.combo.SetFocus()
    ...
    def zJogCmd(self, code, val):
        ...
        if val == 'Cont':
            if self.jogCode != code: # new jog code
                if self.jogCode is None: # jogging stopped
                    ...
                    try:
                        ...
                        self.comm.command(cm.C_Z_J_MOV)
                    except CommTimeout:
                        commTimeout(self)
            else:
                try:
            ...
            if self.jogCode is None:
                ...
                stdout.flush()
                try:
                    self.comm.queParm(pm.Z_FLAG, en.CMD_JOG)
                    self.comm.queParm(pm.Z_MOVE_DIST, val)
                    self.comm.command(cm.C_Z_MOVE_REL)
                except CommTimeout:
                    commTimeout(self)
        ...
        if val == 'Cont':
            if self.jogCode != code:
                if self.jogCode is None:
                    ...
                    try:
                        ...
                        self.comm.command(cm.C_X_J_MOV)
                    except CommTimeout:
                        commTimeout(self)
            else:
                try:
            ...
            if self.jogCode is None:
                ...
                try:
                    ...
                    self.comm.command(cm.C_X_MOVE_REL)
                except CommTimeout:
                    commTimeout(self)
...
class PosMenu(wx.Menu):
    ...
    def OnHomeXFwd(self, _):
        ...
        if not HOME_IN_PLACE:
            self.comm.command(cm.C_X_HOME_FWD)
    ...
    def OnHomeZFwd(self, _):
        jp = self.jP
        if not HOME_IN_PLACE:
            self.comm.command(cm.C_Z_HOME_FWD)
            jp.probe(HOME_Z)
        else:
    ...
    def OnHomeXRev(self, _):
        jp = self.jP
        if not HOME_IN_PLACE:
            self.comm.command(cm.C_X_HOME_REV)
            jp.probe(HOME_X)
        else:
            xLocation = float(jp.xPos.GetValue())
            jp.xHomeOffset = 0 - xLocation
    ...
    def OnHomeZRev(self, _):
        jp = self.jP
        if not HOME_IN_PLACE:
            self.comm.command(cm.C_Z_HOME_REV)
            jp.probe(HOME_Z)
        else:
            zLocation = float(jp.zPos.GetValue())
            jp.zHomeOffset = 0 - zLocation
...
class GotoDialog(wx.Dialog, FormRoutines):
    ...
    def OnOk(self, _):
        try:
            ...
            m.queClear()
            self.comm.command(cm.C_CMD_PAUSE)
            self.comm.command(cm.C_CLEAR_QUE)
            if self.axis == AXIS_Z:
...
class RetractDialog(wx.Dialog, FormRoutines):
    ...
    def OnOk(self, _):
        ...
        try:
            ...
            comm.command(cm.C_CLEAR_QUE)
            if self.axis == AXIS_Z:
                self.jp.mf.sendData.sendZData()
                m.saveZOffset()
                m.moveZ(loc, en.CMD_JOG)
                jp.zReturnLoc = \
...
class UpdateThread(Thread):
    ...
    def readAll(self):
        ...
        try:
            result = self.comm.command(cm.C_READ_ALL, True)
            if result is None:
                return
    ...
    def procDebug(self):
        try:
            result = self.comm.getString(cm.C_READ_DBG, 10)
            if not self.threadRun:
                return(True)
            if result is None:
                return(False)
...
class MainFrame(wx.Frame):
    def __init__(self, parent, title):
        ...
        self.runoutSync = en.SEL_RU_NO_ENC
    ...
    def initDevice(self):
        ...
        if comm.ser is not None:
            try:
                queParm = comm.queParm
                queParm(pm.CFG_FPGA, cfg.getBoolInfoData(cf.cfgFpga))
                # queParm(pm.CFG_FCY, cfg.getInfoData(cf.cfgFcy))
                queParm(pm.CFG_MPG, cfg.getBoolInfoData(cf.cfgMPG))
                queParm(pm.CFG_DRO, cfg.getBoolInfoData(cf.cfgDRO))
                queParm(pm.CFG_LCD, cfg.getBoolInfoData(cf.cfgLCD))
                if not R_PI:
                    queParm(pm.CFG_MEGA, cfg.getBoolInfoData(cf.cfgMega))
                queParm(pm.CFG_SWITCH, cfg.getBoolInfoData(cf.spSwitch))
                queParm(pm.CFG_VAR_SPEED, \
                             cfg.getBoolInfoData(cf.spVarSpeed))
                queParm(pm.FPGA_FREQUENCY, \
                             cfg.getIntInfoData(cf.cfgFpgaFreq))
                queParm(pm.COMMON_LIMITS, \
                             cfg.getBoolInfoData(cf.cfgCommonLimits))
                queParm(pm.LIMITS_ENABLED, \
                             cfg.getBoolInfoData(cf.cfgLimitsEnabled))
                queParm(pm.COMMON_HOME, \
                             cfg.getBoolInfoData(cf.cfgCommonHome))

                queParm(pm.PROBE_INV, cfg.getBoolInfoData(cf.cfgPrbInv))
                queParm(pm.E_STOP_ENA, cfg.getBoolInfoData(cf.cfgEStop))
                queParm(pm.E_STOP_INV, cfg.getBoolInfoData(cf.cfgEStopInv))
                if RISCV:
                    queParm(pm.DRO_STEP, cfg.getBoolInfoData(cf.cfgDROStep))
                ...
                if EXT_DRO:
                    ...
                    queParm(pm.Z_LOC, jp.zPosition)
                    jp.zHomeOffset = cfg.getFloatInfo(cf.zSvHomeOffset)
                    queParm(pm.Z_HOME_OFFSET, \
                    ...
                    if DRO:
                        ...
                        queParm(pm.Z_DRO_LOC, jp.zDROPosition)
                        jp.zDROOffset = cfg.getFloatInfo(cf.zSvDROOffset)
                        queParm(pm.Z_DRO_OFFSET, \
                        ...
                        stdout.flush()
                comm.sendMulti()
                comm.command(cm.C_CMD_Z_SET_LOC)
                ...
                if EXT_DRO:
                    ...
                    queParm(pm.X_LOC, jp.xPosition)
                    jp.xHomeOffset = cfg.getFloatInfo(cf.xSvHomeOffset)
                    queParm(pm.X_HOME_OFFSET, \
                    ...
                    if DRO:
                        ...
                        queParm(pm.X_DRO_LOC, jp.xDROPosition)
                        jp.xDROOffset = cfg.getFloatInfo(cf.xSvDROOffset)
                        queParm(pm.X_DRO_OFFSET, \
                ...
                if HOME_TEST:
                    val = str(int(cfg.getFloatInfoData(cf.xHomeLoc) * \
                                  self.xStepsInch))
                    queParm(pm.X_HOME_LOC, val)
                    queParm(pm.X_HOME_STATUS, \
                                 ct.HOME_SUCCESS if jp.xHomed else \
                                 ct.HOME_ACTIVE)
                comm.command(cm.C_CMD_X_SET_LOC)

                if not R_PI:
                    queParm(pm.MEGA_SIM, cfg.getBoolInfo(cf.spMegaSim))
                comm.sendMulti()
                self.sendData.sendDone()
    ...
    def syncFuncSetup(self):
        ...
        self.runoutSync = runoutSync = cfg.getIntInfoData(cf.cfgRunoutSync)

        syncDbg = True

        if not FPGA:
            if (turnSync == en.SEL_TU_ESYN or \
                threadSync == en.SEL_TH_ESYN or \
                runoutSync == en.SEL_RU_ESYN):
...
class SpindleDialog(wx.Dialog, FormRoutines, DialogActions):
    def __init__(self, mainFrame, defaultFont):
        ...
        self.fields += (
            ...
            ("cRunout Sync", cf.cfgRunoutSync, 'c', self.runoutSync),
        )
        if STEP_DRV or MOTOR_TEST:
            self.fields += (
    ...
    def OnStart(self, _):
        ...
        if not self.mf.senData.spindleDataSent:
            ...
            mf.comm.command(cm.C_CMD_SP_SETUP)
    ...
    def turnSync(self):
        ...
        return( indexList, choiceList, en.selTurnText, self.OnTurnEvent)

    def OnTurnEvent(self, e):
        print("OnTurnEvent")
        combo = e.EventObject
        val = combo.GetValue()
        combo.SetValue(val)
    ...
    def threadSync(self):
        if STEP_DRV:
            ...
            if not FPGA:
                if SPINDLE_INTERNAL_SYNC:
                    indexList += (en.SEL_TH_ISYN,)
                if SPINDLE_SYNC_BOARD:
                    indexList += (en.SEL_TH_ESYN,)
            else:
                indexList += (en.SEL_TH_SYN,)
        else:
        ...
        return (indexList, choiceList, en.selThreadText, self.OnThreadEvent)
    ...
    def runoutSync(self):
        if STEP_DRV:
            indexList = (en.SEL_RU_STEP,)
        elif SPINDLE_ENCODER:
            indexList = (en.SEL_RU_ENC,)
            if not FPGA:
                if SPINDLE_INTERNAL_SYNC:
                    indexList += (en.SEL_RU_ISYN,)
                if SPINDLE_SYNC_BOARD:
                    indexList += (en.SEL_RU_ESYN,)
            else:
                indexList += (en.SEL_RU_SYN,)
        else:
            indexList = (en.SEL_RU_NO_ENC,)

        choiceList = []
        for i in indexList:
            choiceList.append(en.selRunoutText[i])
        return (indexList, choiceList, en.selRunoutText, self.OnRunoutEvent)

    def OnRunoutEvent(self, e):
        print("OnRunoutEvent")
        combo = e.EventObject
        val = combo.GetValue()
        combo.SetValue(val)
