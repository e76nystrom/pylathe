...
ENC_PER_REV          = 141	# 0x8d

# test encoder setup variables

ENC_ENABLE           = 142	# 0x8e
ENC_PRE_SCALER       = 143	# 0x8f
ENC_TIMER            = 144	# 0x90
ENC_RUN_COUNT        = 145	# 0x91

# test encoder status variables

ENC_RUN              = 146	# 0x92
ENC_COUNTER          = 147	# 0x93
ENC_REV_COUNTER      = 148	# 0x94
...
L_SYNC_CYCLE         = 153	# 0x99
L_SYNC_OUTPUT        = 154	# 0x9a
L_SYNC_IN_PRESCALER  = 155	# 0x9b
L_SYNC_OUT_PRESCALER = 156	# 0x9c

# x sync parameters

L_X_SYNC_CYCLE       = 157	# 0x9d
L_X_SYNC_OUTPUT      = 158	# 0x9e
L_X_SYNC_IN_PRESCALER = 159	# 0x9f
L_X_SYNC_OUT_PRESCALER = 160	# 0xa0

# threading variables

TH_Z_START           = 161	# 0xa1
TH_X_START           = 162	# 0xa2
TAN_THREAD_ANGLE     = 163	# 0xa3
X_FEED               = 164	# 0xa4
RUNOUT_DISTANCE      = 165	# 0xa5
RUNOUT_DEPTH         = 166	# 0xa6
...
LIMIT_OVERRIDE       = 172	# 0xac
COMMON_LIMITS        = 173	# 0xad
LIMITS_ENABLED       = 174	# 0xae
COMMON_HOME          = 175	# 0xaf

# z limits and home

Z_LIM_ENA            = 176	# 0xb0
Z_LIM_NEG_INV        = 177	# 0xb1
Z_LIM_POS_INV        = 178	# 0xb2
Z_HOME_ENA           = 179	# 0xb3
Z_HOME_INV           = 180	# 0xb4
...
X_LIM_ENA            = 181	# 0xb5
X_LIM_NEG_INV        = 182	# 0xb6
X_LIM_POS_INV        = 183	# 0xb7
X_HOME_ENA           = 184	# 0xb8
X_HOME_INV           = 185	# 0xb9
...
ARC_RADIUS           = 189	# 0xbd
ARC_X_CENTER         = 190	# 0xbe
ARC_Z_CENTER         = 191	# 0xbf
ARC_X_START          = 192	# 0xc0
ARC_Z_START          = 193	# 0xc1
ARC_X_END            = 194	# 0xc2
ARC_Z_END            = 195	# 0xc3
MEGA_VFD             = 196	# 0xc4
MEGA_SIM             = 197	# 0xc5
...
remParmTable = ( \
    ...
    ("CAP_TMR_ENABLE",         "char",     "capTmrEnable"       ), # 0x83 131
    ("CFG_FPGA",               "char",     "cfgFpga"            ), # 0x84 132
    ("CFG_MEGA",               "char",     "cfgMega"            ), # 0x85 133
    ("CFG_MPG",                "char",     "cfgMpg"             ), # 0x86 134
    ("CFG_DRO",                "char",     "cfgDro"             ), # 0x87 135
    ("CFG_LCD",                "char",     "cfgLcd"             ), # 0x88 136
    ("CFG_FCY",                "uint_t",   "cfgFcy"             ), # 0x89 137
    ("CFG_SWITCH",             "int",      "cfgSwitch"          ), # 0x8a 138
    ("CFG_VAR_SPEED",          "int",      "cfgVarSpeed"        ), # 0x8b 139
    ("SETUP_DONE",             "char",     "setupDone"          ), # 0x8c 140
    ("ENC_PER_REV",            "uint16_t", "encPerRev"          ), # 0x8d 141
    ("ENC_ENABLE",             "char",     "encEnable"          ), # 0x8e 142
    ("ENC_PRE_SCALER",         "uint16_t", "encPreScaler"       ), # 0x8f 143
    ("ENC_TIMER",              "uint16_t", "encTimer"           ), # 0x90 144
    ("ENC_RUN_COUNT",          "int",      "encRunCount"        ), # 0x91 145
    ("ENC_RUN",                "char",     "encRun"             ), # 0x92 146
    ("ENC_COUNTER",            "int16_t",  "encCounter"         ), # 0x93 147
    ("ENC_REV_COUNTER",        "int32_t",  "encRevCounter"      ), # 0x94 148
    ("RPM",                    "int16_t",  "rpm"                ), # 0x95 149
    ("FPGA_FREQUENCY",         "int32_t",  "fpgaFrequency"      ), # 0x96 150
    ("FREQ_MULT",              "int16_t",  "freqMult"           ), # 0x97 151
    ("X_CFG_REG",              "int16_t",  "xCfgReg"            ), # 0x98 152
    ("L_SYNC_CYCLE",           "uint16_t", "lSyncCycle"         ), # 0x99 153
    ("L_SYNC_OUTPUT",          "uint16_t", "lSyncOutput"        ), # 0x9a 154
    ("L_SYNC_IN_PRESCALER",    "uint16_t", "lSyncInPrescaler"   ), # 0x9b 155
    ("L_SYNC_OUT_PRESCALER",   "uint16_t", "lSyncOutPrescaler"  ), # 0x9c 156
    ("L_X_SYNC_CYCLE",         "uint16_t", "lXSyncCycle"        ), # 0x9d 157
    ("L_X_SYNC_OUTPUT",        "uint16_t", "lXSyncOutput"       ), # 0x9e 158
    ("L_X_SYNC_IN_PRESCALER",  "uint16_t", "lXSyncInPrescaler"  ), # 0x9f 159
    ("L_X_SYNC_OUT_PRESCALER", "uint16_t", "lXSyncOutPrescaler" ), # 0xa0 160
    ("TH_Z_START",             "int32_t",  "thZStart"           ), # 0xa1 161
    ("TH_X_START",             "int32_t",  "thXStart"           ), # 0xa2 162
    ("TAN_THREAD_ANGLE",       "float",    "tanThreadAngle"     ), # 0xa3 163
    ("X_FEED",                 "int32_t",  "xFeed"              ), # 0xa4 164
    ("RUNOUT_DISTANCE",        "float",    "runoutDistance"     ), # 0xa5 165
    ("RUNOUT_DEPTH",           "float",    "runoutDepth"        ), # 0xa6 166
    ("JOG_DEBUG",              "char",     "jogDebug"           ), # 0xa7 167
    ("PWM_FREQ",               "uint_t",   "pwmFreq"            ), # 0xa8 168
    ("MIN_SPEED",              "int16_t",  "minSpeed"           ), # 0xa9 169
    ("MAX_SPEED",              "int16_t",  "maxSpeed"           ), # 0xaa 170
    ("CURRENT_OP",             "char",     "currentOp"          ), # 0xab 171
    ("LIMIT_OVERRIDE",         "char",     "limitOverride"      ), # 0xac 172
    ("COMMON_LIMITS",          "char",     "commonLimits"       ), # 0xad 173
    ("LIMITS_ENABLED",         "char",     "limitsEnabled"      ), # 0xae 174
    ("COMMON_HOME",            "char",     "commonHome"         ), # 0xaf 175
    ("Z_LIM_ENA",              "char",     "zLimEna"            ), # 0xb0 176
    ("Z_LIM_NEG_INV",          "char",     "zLimNegInv"         ), # 0xb1 177
    ("Z_LIM_POS_INV",          "char",     "zLimPosInv"         ), # 0xb2 178
    ("Z_HOME_ENA",             "char",     "zHomeEna"           ), # 0xb3 179
    ("Z_HOME_INV",             "char",     "zHomeInv"           ), # 0xb4 180
    ("X_LIM_ENA",              "char",     "xLimEna"            ), # 0xb5 181
    ("X_LIM_NEG_INV",          "char",     "xLimNegInv"         ), # 0xb6 182
    ("X_LIM_POS_INV",          "char",     "xLimPosInv"         ), # 0xb7 183
    ("X_HOME_ENA",             "char",     "xHomeEna"           ), # 0xb8 184
    ("X_HOME_INV",             "char",     "xHomeInv"           ), # 0xb9 185
    ("E_STOP_ENA",             "char",     "eStopEna"           ), # 0xba 186
    ("E_STOP_INV",             "char",     "eStopInv"           ), # 0xbb 187
    ("CMD_PAUSED",             "char",     "cmdPaused"          ), # 0xbc 188
    ("ARC_RADIUS",             "float",    "arcRadius"          ), # 0xbd 189
    ("ARC_X_CENTER",           "int",      "arcXCenter"         ), # 0xbe 190
    ("ARC_Z_CENTER",           "int",      "arcZCenter"         ), # 0xbf 191
    ("ARC_X_START",            "int",      "arcXStart"          ), # 0xc0 192
    ("ARC_Z_START",            "int",      "arcZStart"          ), # 0xc1 193
    ("ARC_X_END",              "int",      "arcXEnd"            ), # 0xc2 194
    ("ARC_Z_END",              "int",      "arcZEnd"            ), # 0xc3 195
    ("MEGA_VFD",               "char",     "megaVfd"            ), # 0xc4 196
    ("MEGA_SIM",               "char",     "megaSim"            ), # 0xc5 197
    ("USB_ENA",                "char",     "usbEna"             ), # 0xc6 198
    ("DRO_STEP",               "char",     "droStep"            ), # 0xc7 199
    ("MAX_PARM",               "int16_t",  "maxParm"            ), # 0xc8 200
    )
